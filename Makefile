...
# containing files matching the given pattern ($2)
...
ifeq (,$(filter clean clean-%,$(MAKECMDGOALS)))
  ...
  $1_CLASSPATH_$3 := "\`dirname \$$$$0\`/j$1_$3.jar:$(PROJECT_DIR)/$(BUILD_LIB_DIR)/j$1_$3.jar$(LIB_TARGETS:%=:$(PROJECT_DIR)/$(BUILD_LIB_DIR)/%_$3-$(VERSION).zip)$(LIB_TARGETS:%=:$(INSTALL_LIB_DIR)/%_$3-$(VERSION).zip):$(CLASSPATH_$3)"
  endef
  ...
  TEST_$1_CLASSPATH := "\`dirname \$$$$0\`/j$1.jar:$(PROJECT_DIR)/$(BUILD_LIB_DIR)/j$1.jar$(LIB_TARGETS:%=:$(PROJECT_DIR)/$(BUILD_LIB_DIR)/%_s-$(VERSION).zip):$(CLASSPATH_$3)"
  endef
...
.Olib_$1/lib/$2/%.o: $(SRC_DIR)/lib/$2/%.scm
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) $$(BFLAGS_$1) $$(BCFLAGS_$1:%=-copt %) $$(BLFLAGS_$1) $$(LIB_$2_INCLUDE_FLAGS) $$< -o $$@ -c

.class_$1/bigloo/lib/$2/%.class: $(SRC_DIR)/lib/$2/%.scm
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) -jvm $$(BFLAGS_$1) $$(BLFLAGS_$1) $$(LIB_$2_INCLUDE_FLAGS) $$< -o $$@ -c
endef

define scm_bin_pattern_rules
.Olib_$1/bin/$2/%.o: $(SRC_DIR)/bin/$2/%.scm
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) $$(BFLAGS_$1) $$(BCFLAGS_$1:%=-copt %) $$(BLFLAGS_$1) $$($2_INCLUDE_FLAGS) $$< -o $$@ -c

.class_$1/bigloo/bin/$2/%.class: $(SRC_DIR)/bin/$2/%.scm
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) -jvm $$(BFLAGS_$1) $$(BLFLAGS_$1) $$($2_INCLUDE_FLAGS) $$< -o $$@ -c
endef

define scm_test_pattern_rules
.Olib_$1/$2/%.o: $(TEST_DIR)/$2/%.scm
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) $$(BFLAGS_$1) $$(BCFLAGS_$1:%=-copt %) $$(BLFLAGS_$1) $$(TEST_$2_INCLUDE_FLAGS) $$< -o $$@ -c

.class_$1/bigloo/test/$2/%.class: $(TEST_DIR)/$2/%.scm
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) -jvm $$(BFLAGS_$1) $$(BLFLAGS_$1) $$(TEST_$2_INCLUDE_FLAGS) $$< -o $$@ -c                   
...
$(BUILD_BIN_DIR)/$1_$2: $(AFILE) $$($1_OBJECTS_$2) | release_libs
	@echo "building $$@..."
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) -o $$@  $$(BFLAGS_$2) $$(BLFLAGS_$2) $$($1_INCLUDE_FLAGS) $$($1_OBJECTS_$2)
endef
...
$(BUILD_BIN_DIR)/j$1_$2:  $$($1_CLASSES_$2) | $(JFILE) release_zips
	...
	@$$(BIGLOO) -jvm  -jvm-classpath $$($1_CLASSPATH_$2) -o $$@ -jfile $(JFILE) $$(BFLAGS_$2) \
                    $$(BLFLAGS_$2) $$($1_INCLUDE_FLAGS)  $$($1_CLASSES_$2:.class_$2/%=%)
endef
...
$(TEST_BUILD_DIR)/$1: $(AFILE) $$(TEST_$1_OBJECTS) | release_libs
	@echo "building $$@..."
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$$(BIGLOO) -o $$@  $$(BFLAGS_s) $$(BLFLAGS_s) $$(TEST_$1_INCLUDE_FLAGS) $$(TEST_$1_OBJECTS)
endef
...
$(TEST_BUILD_DIR)/j$1: $(AFILE) $(JFILE) $$(TEST_$1_CLASSES) | release_zips
	...
	@$$(BIGLOO) -jvm  -jvm-classpath $$(TEST_$1_CLASSPATH) -o $$@ -jfile $(JFILE) $$(BFLAGS_s) \
                    $$(BLFLAGS_s) $$(TEST_$1_INCLUDE_FLAGS)  $$(TEST_$1_CLASSES:.class_s/%=%)
endef
...
$(BUILD_LIB_DIR)/lib$1_$2-$(VERSION).$(SHARED_LIBRARY_EXT): $(AFILE) $(BUILD_LIB_DIR)/$1.init $(BUILD_LIB_DIR)/$1.heap $$(LIB_$1_OBJECTS_$2)
	@echo "building $$@..."
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$(BIGLOO) -y -o $$@  $$(BLFLAGS_$2) $$(LIB_$1_INCLUDE_FLAGS) -ldpostopt -Wl,-soname=`basename $$@` $$(LIB_$1_OBJECTS_$2)
...
$(BUILD_LIB_DIR)/lib$1_e$2-$(VERSION).$(SHARED_LIBRARY_EXT): $(AFILE) $(BUILD_LIB_DIR)/$1.init $(BUILD_LIB_DIR)/$1.heap $$(LIB_$1_HEAP_OBJECTS_$2)  
	@echo "building $$@..."
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$(BIGLOO) -y -o $$@  $$(BLFLAGS_$2) $$(LIB_$1_INCLUDE_FLAGS) -ldpostopt -Wl,-soname=`basename $$@` $$(LIB_$1_HEAP_OBJECTS_$2)
...
$(BUILD_LIB_DIR)/lib$1_$2_mt-$(VERSION).$(SHARED_LIBRARY_EXT): $(AFILE) $(BUILD_LIB_DIR)/$1.init $(BUILD_LIB_DIR)/$1.heap $$(LIB_$1_OBJECTS_$2) 
	...
	@$(BIGLOO) -y -o $$@ -eval '(set! *multi-threaded-gc?* #t)' \
                   $$(BLFLAGS_$2) $$(LIB_$1_INCLUDE_FLAGS)  -ldpostopt -Wl,-soname=`basename $$@` $$(LIB_$1_OBJECTS_$2)
...
$(BUILD_LIB_DIR)/lib$1_e$2_mt-$(VERSION).$(SHARED_LIBRARY_EXT): $(AFILE) $(BUILD_LIB_DIR)/$1.init $(BUILD_LIB_DIR)/$1.heap $$(LIB_$1_HEAP_OBJECTS_$2)
	...
	@if [ ! -d "$$(@D)" ]; then  mkdir -p $$(@D); fi
	@$(BIGLOO) -y -o $$@ -eval '(set! *multi-threaded-gc?* #t)' \
                   $$(BLFLAGS_$2) $$(LIB_$1_INCLUDE_FLAGS)  -ldpostopt -Wl,-soname=`basename $$@` $$(LIB_$1_HEAP_OBJECTS_$2)
endef
